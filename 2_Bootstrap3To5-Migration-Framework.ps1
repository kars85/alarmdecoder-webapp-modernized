        ...
        Get-ChildItem -Path $TargetDirectory -Recurse -Include $ext -File -ErrorAction SilentlyContinue | ForEach-Object {
            ...
            try {
                ...
                if ($originalContent -ne $content) {
                    ...
                    if (-not $DryRun) {
                        # CORRECTED: Use the string name for the encoding
                        # CORRECTED AGAIN: Use the standard UTF8 identifier recognized by Set-Content
                        Set-Content -Path $file -Value $content -Encoding UTF8 -NoNewline -ErrorAction Stop
...
function Test-MigrationResults {
    ...
    $bootstrap3_4Patterns = @(
        ...
        # CORRECTED: Find data-toggle where value does NOT start with bs-
        '\bdata-toggle="(?!bs-)[^"]+"',
