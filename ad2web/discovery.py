                ...
                for addr_info in ipv4_addrs:
                    ...
                    if (
                        ...
                        if iface == default_gw_iface:
                            ...
                            logger.debug(
                                f"  IP {preferred_ip} matches gateway interface {iface}. Using this."
                            )
                            break
                if preferred_ip:
                    break
    ...
    for target_ip, target_port in targets:
        try:
            ...
            if ip_address and not ip_address.startswith("127."):
                logger.debug(
                    f"Socket method succeeded using target {target_ip}:{target_port}. IP: {ip_address}"
                )
                s.close()
                return ip_address
            ...
            logger.warning(
                f"Unexpected error getting IP via socket with target {target_ip}:{target_port}: {e}"
            )
            if s:
                s.close()
            continue
...
class DiscoveryServer(threading.Thread):
    ...
    def run(self):
        ...
        with self._decoder.app.app_context():
            ...
            if not self._current_ip_address:
                self.logger.error(
                    "DiscoveryServer could not determine local IP address. SSDP responses may be incorrect."
                )
            else:
                self.logger.info(
                    f"Discovery running: loc=http://{self._current_ip_address}:{self._current_port}/..., uuid={self._device_uuid}"
                )
    ...
    def _handle_request(self, request, addr):
        if request.error_code:
            self.logger.warning(
                f"Discovery Parse Error from {addr}: {request.error_code} - {request.error_message}"
            )
            return
        ...
        if self._match_search_request(request):
            self.logger.info(
                f"Received matching M-SEARCH from {addr} for ST: {request.headers.get('ST', 'N/A')}"
            )
            ...
            self.logger.debug(
                f"Ignoring non-matching M-SEARCH from {addr} for ST: {request.headers.get('ST', 'N/A')}"
            )
    ...
    def _update(self):
        if self._current_ip_address:
            ...
            if current_ip and current_ip != self._current_ip_address:
                self.logger.info(
                    f"IP address changed from {self._current_ip_address} to {current_ip}. Updating configuration."
                )
            ...
            if self._announcement_timestamp + (self._expiration_time / 2.0) < now:
                if self._current_ip_address and self._device_uuid:
                    ...
                    for msg in notify_messages:
                        self._send_message(
                            msg.encode("utf-8"), (self.MCAST_ADDRESS, self.MCAST_PORT)
                        )
    ...
    def _create_discovery_response(self, request):
        ...
        loc = (
            f"http://{self._current_ip_address}:{self._current_port}/static/device_description.xml"
        )
    ...
    def _send_message(self, message_bytes, addr):
        ...
        for i in range(2):
            try:
                ...
                self.logger.error(
                    f"Unexpected error sending to {addr} (attempt {i+1}/2): {e}", exc_info=True
                )
                break
    ...
    def _match_search_request(self, request):
        ...
        if request.command == "M-SEARCH" and request.path == "*" and man == '"ssdp: discover"':
            if st in {
                "ssdp: all",
                "upnp:rootdevice",
                f"uuid:{self._device_uuid}",
                "urn:schemas-upnp-org:device:AlarmDecoder:1",
            }:
                return True
        return False
    ...
    def _get_device_uuid(self):
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get or generate device UUID from database: {e}", exc_info=True
            )
            device_uuid = None
        return device_uuid
