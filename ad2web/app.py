    ...
    def __call__(self, environ, start_response):
        ...
        if script_name:
            ...
            if original_path_info.startswith(script_name):
                 ...
                 environ['PATH_INFO'] = new_path_info if new_path_info.startswith('/') else f'/{new_path_info}'
...
def create_app(config=None, app_name=None, blueprints=None):
    ...
    flask_app.wsgi_app = ReverseProxied(flask_app.wsgi_app, num_proxies=flask_app.config.get('NUM_PROXIES', 1))
    ...
    appsocket = create_decoder_socket(flask_app) # Pass app, not socketio
...
def configure_logging(app):
    ...
    stream_formatter = logging.Formatter('%(asctime)s %(levelname)s: %(message)s [in %(pathname)s:%(lineno)d]')
        ...
        if not app.testing:
            if request.blueprint not in safe_blueprints:
                try:
                    ...
                    app.logger.error(f"Error querying setup_stage in before_request: {e}", exc_info=True)
                    setup_stage = None

                if setup_stage is None:
                     # Redirect to setup if not complete and not already in setup
                     if request.endpoint not in {'setup.index', 'setup.type'}: # Allow initial setup pages
                         app.logger.debug(f"Setup not started. Redirecting to setup.index (from {request.endpoint})")
                    ...
                    if request.blueprint != 'setup' or (required_stage and required_stage > setup_stage):
                        # Redirect to the correct setup stage page
                        target_endpoint = SETUP_STAGE_ENDPOINT.get(setup_stage, 'setup.index')
                        app.logger.debug(f"Setup stage {setup_stage} required, redirecting to {target_endpoint} (from {request.endpoint})")
...
def configure_error_handlers(app):
    ...
    def server_error_page(error):
        ...
        try:
            ...
            app.logger.error(f"!!! Additionally failed to render 500 template: {render_err}", exc_info=True)
            return "<h1>Server Error</h1><p>An internal server error occurred, and the error page could not be rendered.</p>", 500
