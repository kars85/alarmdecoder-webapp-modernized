    ...
    def addportforward(self, internal_port, external_port):
        try:
            if has_upnp:
                ...
                port_result = self.upnp.addportmapping(
                    external_port,
                    "TCP",
                    self.upnp.lanaddr,
                    internal_port,
                ...
                with self._decoder.app.app_context():
                    self._decoder.app.logger.info(
                        "Port Forward Attempt: Discovery={0}, IGD={1}, Result={2}".format(
                            discover, igd, port_result
                        )
                    )
            else:
    ...
    def removeportforward(self, external_port):
        try:
            if has_upnp:
                ...
                with self._decoder.app.app_context():
                    self._decoder.app.logger.info(
                        "Port Delete Attempt: Discovery={0}, IGD={1}, Result={2}".format(
                            discover, igd, port_result
                        )
                    )
            else:
