...
def check_time_restriction(start_time, end_time):
    ...
    start_time = message_time.replace(
        hour=int(st[0]), minute=int(st[1]), second=int(st[2]), microsecond=0
    )
    end_time = message_time.replace(
        hour=int(et[0]), minute=int(et[1]), second=int(et[2]), microsecond=0
    )
