...
class TimeSettingsInternalForm(Form):
    starttime = StringField(
        "Start Time",
        [DataRequired(), Length(max=8), TimeValidator()],
        default="00:00:00",
        description="Start time for this event notification (24hr format)",
    )
    endtime = StringField(
        "End Time",
        [DataRequired(), Length(max=8), TimeValidator()],
        default="23:59:59",
        description="End time for this event notification (24hr format)",
    )
    delaytime = IntegerField(
        "Zone Tracker Notification Delay",
        [DataRequired(), NumberRange(min=0)],
        default=0,
        description="Time in minutes to delay sending Zone Tracker notification",
    )
    suppress = BooleanField(
        "Suppress Zone Tracker Restore?",
        [Optional()],
        description="Suppress Zone Tracker notification if restored before delay",
    )
...
class EditNotificationForm(Form):
    ...
    suppress_timestamp = BooleanField(
        "Suppress Timestamp?",
        [Optional()],
        description="Removes Timestamp from Message Body and Subject",
    )
    time_field = FormField(TimeSettingsInternalForm)
    subscriptions = MultiCheckboxField(
        "Notification Events", choices=[(str(k), v) for k, v in SUBSCRIPTIONS.items()]
    )

    def populate_settings(self, settings, id=None):
        settings["subscriptions"] = self.populate_setting(
            "subscriptions", json.dumps({str(k): True for k in self.subscriptions.data})
        )
    ...
    def populate_from_settings(self, id):
        ...
        if subscriptions:
            self.subscriptions.data = [
                k if v == True else False for k, v in json.loads(subscriptions).items()
            ]
...
class EmailNotificationInternalForm(Form):
    source = StringField(
        ...
        description="Emails will originate from this address",
    )
    ...
    subject = StringField(
        "Email Subject",
        [DataRequired(), Length(max=255)],
        default="AlarmDecoder: Alarm Event",
        description="Emails will contain this text as the subject",
    )

    server = StringField(
        "Email Server (Configured using local server by default, not preferred due to ISP filtering)",
        [DataRequired(), Length(max=255)],
        default="localhost",
    )
    port = IntegerField(
        "Server Port (If using your own server, check that port is not filtered by ISP)",
...
class EmailNotificationForm(EditNotificationForm):
    ...
    def populate_settings(self, settings, id=None):
        ...
        settings["destination"] = self.populate_setting(
            "destination", self.form_field.destination.data
        )
        ...
        settings["authentication_required"] = self.populate_setting(
            "authentication_required", self.form_field.authentication_required.data
        )
    ...
    def populate_from_settings(self, id):
        ...
        self.form_field.authentication_required.data = self.populate_from_setting(
...
class PushoverNotificationInternalForm(Form):
    token = StringField(
        "API Token", [DataRequired(), Length(max=30)], description="Your Application's API Token"
    )
    user_key = StringField(
        "User/Group Key", [DataRequired(), Length(max=30)], description="Your user or group key"
    )
    priority = SelectField(
        "Message Priority",
        choices=[
            PUSHOVER_PRIORITIES[LOWEST],
            PUSHOVER_PRIORITIES[LOW],
            PUSHOVER_PRIORITIES[NORMAL],
            PUSHOVER_PRIORITIES[HIGH],
            PUSHOVER_PRIORITIES[EMERGENCY],
        ],
        default=PUSHOVER_PRIORITIES[LOW],
        description="Pushover message priority",
        coerce=int,
    )
    title = StringField(
        "Title of Message", [Length(max=255)], description="Title of Notification Messages"
    )
...
class TwilioNotificationInternalForm(Form):
    account_sid = StringField(
        "Account SID", [DataRequired(), Length(max=50)], description="Your Twilio Account SID"
    )
    auth_token = StringField(
        "Auth Token", [DataRequired(), Length(max=50)], description="Your Twilio User Auth Token"
    )
    number_to = StringField(
        "To", [DataRequired(), Length(max=15)], description="Number to send SMS/call to"
    )
    number_from = StringField(
        "From", [DataRequired(), Length(max=15)], description="Must Be A Valid Twilio Phone Number"
    )
...
class TwilioNotificationForm(EditNotificationForm):
    ...
    def populate_settings(self, settings, id=None):
        EditNotificationForm.populate_settings(self, settings, id)
        settings["account_sid"] = self.populate_setting(
            "account_sid", self.form_field.account_sid.data
        )
        settings["auth_token"] = self.populate_setting(
            "auth_token", self.form_field.auth_token.data
        )
        settings["number_to"] = self.populate_setting("number_to", self.form_field.number_to.data)
        settings["number_from"] = self.populate_setting(
            "number_from", self.form_field.number_from.data
        )
...
class TwiMLNotificationInternalForm(Form):
    ...
    twimlet_url = StringField(
        "Twimlet URL",
        [DataRequired()],
        default="http://twimlets.com/message",
        description="Your twimlet URL (http://twimlets.com/message)",
    )
...
class TwiMLNotificationForm(EditNotificationForm):
    ...
    def populate_settings(self, settings, id=None):
        ...
        settings["twimlet_url"] = self.populate_setting(
            "twimlet_url", self.form_field.twimlet_url.data
        )
...
class ProwlNotificationInternalForm(Form):
    prowl_api_key = StringField(
        "API Key", [DataRequired(), Length(max=50)], description="Your Prowl API Key"
    )
    prowl_app_name = StringField(
        "Application Name",
        [DataRequired(), Length(max=256)],
        description="Application Name to Show in Notifications",
        default="AlarmDecoder",
    )
    prowl_priority = SelectField(
        "Message Priority",
        choices=[
            PROWL_PRIORITIES[LOWEST],
            PROWL_PRIORITIES[LOW],
            PROWL_PRIORITIES[NORMAL],
            PROWL_PRIORITIES[HIGH],
            PROWL_PRIORITIES[EMERGENCY],
        ],
        default=PROWL_PRIORITIES[LOW],
        description="Prowl message priority",
        coerce=int,
    )
...
class ProwlNotificationForm(EditNotificationForm):
    ...
    def populate_settings(self, settings, id=None):
        ...
        settings["prowl_api_key"] = self.populate_setting(
            "prowl_api_key", self.form_field.prowl_api_key.data
        )
        settings["prowl_app_name"] = self.populate_setting(
            "prowl_app_name", self.form_field.prowl_app_name.data
        )
        settings["prowl_priority"] = self.populate_setting(
            "prowl_priority", self.form_field.prowl_priority.data
        )
...
class GrowlNotificationInternalForm(Form):
    growl_hostname = StringField(
        "Hostname",
        [DataRequired(), Length(max=255)],
        description="Growl server to send notification to",
    )
    growl_port = StringField(
        "Port", [DataRequired(), Length(max=10)], description="Growl server port", default=23053
    )
    growl_password = PasswordField("Password", description="The password for the growl server")
    growl_title = StringField(
        "Title",
        [DataRequired(), Length(max=255)],
        description="Notification Title",
        default=GROWL_TITLE,
    )
    growl_priority = SelectField(
        "Message Priority",
        choices=[
            GROWL_PRIORITIES[LOWEST],
            GROWL_PRIORITIES[LOW],
            GROWL_PRIORITIES[NORMAL],
            GROWL_PRIORITIES[HIGH],
            GROWL_PRIORITIES[EMERGENCY],
        ],
        default=GROWL_PRIORITIES[LOW],
        description="Growl message priority",
        coerce=int,
    )
...
class GrowlNotificationForm(EditNotificationForm):
    ...
    def populate_settings(self, settings, id=None):
        ...
        settings["growl_hostname"] = self.populate_setting(
            "growl_hostname", self.form_field.growl_hostname.data
        )
        settings["growl_port"] = self.populate_setting(
            "growl_port", self.form_field.growl_port.data
        )
        settings["growl_password"] = self.populate_setting(
            "growl_password", self.form_field.growl_password.data
        )
        settings["growl_title"] = self.populate_setting(
            "growl_title", self.form_field.growl_title.data
        )
        settings["growl_priority"] = self.populate_setting(
            "growl_priority", self.form_field.growl_title.data
        )
...
class CustomPostInternalForm(Form):
    custom_url = StringField(
        "URL",
        [DataRequired(), Length(max=255)],
        description="URL to send data to (ex: www.alarmdecoder.com)",
    )
    custom_path = StringField(
        "Path",
        [DataRequired(), Length(max=400)],
        description="Path to send variables to (ex: /publicapi/add)",
    )
    is_ssl = BooleanField("SSL?", default=False, description="Is the URL SSL or No?")
    method = RadioField(
        "Method",
        choices=[(CUSTOM_METHOD_POST, "POST"), (CUSTOM_METHOD_GET_TYPE, "GET")],
        default=CUSTOM_METHOD_POST,
        coerce=int,
    )
    post_type = RadioField(
        "Type",
        choices=[(URLENCODE, "urlencoded"), (JSON, "JSON"), (XML, "XML")],
        default=URLENCODE,
        coerce=int,
    )
    require_auth = BooleanField(
        "Basic Auth?", default=False, description="Does the URL require basic authentication?"
    )
    auth_username = StringField(
        "Username", [Optional(), Length(max=255)], description="Username for Basic Authentication"
    )
    auth_password = PasswordField(
        "Password", [Optional(), Length(max=255)], description="Password for Basic Authentication"
    )
...
class CustomPostForm(EditNotificationForm):
    ...
    def populate_settings(self, settings, id=None):
        ...
        settings["custom_url"] = self.populate_setting(
            "custom_url", self.form_field.custom_url.data
        )
        settings["custom_path"] = self.populate_setting(
            "custom_path", self.form_field.custom_path.data
        )
        ...
        settings["require_auth"] = self.populate_setting(
            "require_auth", self.form_field.require_auth.data
        )
        settings["auth_username"] = self.populate_setting(
            "auth_username", self.form_field.auth_username.data
        )
        settings["auth_password"] = self.populate_setting(
            "auth_password", self.form_field.auth_password.data
        )
...
class MatrixNotificationInternalForm(Form):
    domain = StringField(
        "Domain",
        [DataRequired(), Length(max=255)],
        description="Domain or IP of matrix server ex. matrix.org",
    )
    room_id = StringField(
        "Room ID",
        [DataRequired(), Length(max=300)],
        description="Room ID and domain ex. !DPNBnAVwxPMvNKTvvY:matrix.org",
    )
    token = StringField(
        "Token",
        [DataRequired(), Length(max=300)],
        description="The long device authentication token. ex. D0gMQowMDI.....",
    )
    custom_values = FieldList(FormField(CustomValueForm), validators=[Optional()], label=None)
    add_field = ButtonField("Add Field", onclick="addField();")
